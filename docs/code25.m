...
% Lund University
%
% Example code 25: Example of dual-input transfer function.
%
% Reference: 
...
xlim([800 N-100])
title( sprintf('%i-step predictions of x_1(t)',k))
...
line( [pstart pstart], figProp.CurrentAxes.YLim, 'Color', 'red', 'LineStyle', ':' )
xlim([800 N-100])
title( sprintf('%i-step predictions of x_2(t)',k))
...
line( [pstart pstart], figProp.CurrentAxes.YLim, 'Color', 'red', 'LineStyle', ':' )
xlim([800 N-100])
title( sprintf('%i-step predictions of y(t)',k))
legend('y(t)', 'Predicted data', 'Prediction starts','Location','NW')


%% Check the resulting residuals.
ehat = y - yhatk;
ehat = ehat(pstart:end);                                % Remove the modelling data.

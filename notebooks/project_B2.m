...
[Fx, Gx] = polydiv(sarima_x.c, sarima_x.a, k);
throw = max(length(Gx), length(sarima_x.c));
xhat_k = filter(Gx, sarima_x.c, log(xm_xv_xt+constant));
xhat_k_org = exp(xhat_k)-constant;

figure
plot(x_t(end-length(xm_xv_xt)+1:end), [xm_xv_xt xhat_k_org] )
line( [y_t(testlim) y_t(testlim)], [-1e6 1e6 ], 'Color','red','LineStyle',':' )
legend('Reconstructed input rain', 'Predicted rain', 'Prediction starts')
title( sprintf('Predicted input signal, x_{t+%i|t}', k) )
axis([x_t(end-length(xv)) x_t(end) min(xm_xv)*1.25 max(xm_xv)*1.25])

%% 3.3.2 Predict the input
% Form the residual for the test data
ehat = xm_xv_xt - xhat_k_org;
ehat = ehat(modelLim:end);
var_ehat = var(ehat)
var_ehat_norm = var(ehat)/var(xt)

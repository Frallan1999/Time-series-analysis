...
%A2 = [1 0 0];
A2 = [1 0 0]; % r = 2 
% THIS ONE WORKS PERFECTLY B = [0 0 1 0 0 0]; % d = 2, s = 3 
B = [0 1 0 0 0]; %d = 1, s = 3 

Mi = idpoly ([1] ,[B] ,[] ,[] ,[A2]);
%Mi.Structure.B.Free = [0 0 0 0 1 1];
...
%FORTSÄTT EXPERIMENTERA MED C för att få den white. Jag skulle ta den andra
%C annars om jag hade haft en annan delay
A1 = [1 0 zeros(1,34) -1]; 
%C1 = [1 zeros(1,5) 0 0 0 0];
C1 = [1 zeros(1,5) 0];
model_init = idpoly (A1, [], C1);
model_init.Structure.A.Free = [0 1 1 zeros(1,33) 1];
model_init.Structure.C.Free = [zeros(1,6) 1];
%model_init.Structure.C.Free = [zeros(1,6) 1 0 0 1];
...
checkIfNormal(res_tilde.y, 'res tilde','D') %White enough
...
Mi.Structure.D.Free = [0 1 1 zeros(1,33) 1];
%Mi.Structure.C.Free = [zeros(1,6) 1 0 0 1];
%Mi.Structure.B.Free = [0 0 0 0 1 1];

%After changing delay
Mi.Structure.C.Free = [zeros(1,6) 1];
Mi.Structure.B.Free = [0 0 1 1 1];
...
%% Removing insignificant variables from above:
